...
export default () => {
    ...
    useEffect(() => {
        ...
        {
            let editAppointment= appointments.find(item=>item.id==id)
            setAppointment(editAppointment)
            setBirthday(new Date(editAppointment.date_of_birth))
            setPreview(editAppointment.avatar)
        }
    },[]);
    const [_birthday, setBirthday] = useState(moment().format("YYYY-MM-DD"))
    useEffect(()=>{
        setAppointment(prevState=>({...prevState,date_of_birth:moment(_birthday).format("YYYY-MM-DD")}))
    },[_birthday])
    ...
    const {  full_name,password,email,phone,info,address,city,gender,date_of_birth,emergency_info,avatar } = _appointment
    const addAppointment= async ()=>{
        ...
        let response = await Appointment.addStaff(appointment)
    ...
    const updateAppointment = async ()=>{
        ...
        let _Edit_Appointment= {..._appointment}
        _Edit_Appointment.avatar = preview
        let res = await Appointment.editStaff(_Edit_Appointment)
        if(res.result == "okie")
        {
            setSaveSuccess(true)
    ...
    // console.log("appointmentGroups",appointmentGroups)
        ...
        <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
            <div className="d-block mb-4 mb-xl-0">
                <Breadcrumb className="d-none d-md-inline-block" listProps={{ className: "breadcrumb-dark breadcrumb-transparent" }}>
                    ...
                    <Breadcrumb.Item >Quản lý lịch khám</Breadcrumb.Item>
                    <Breadcrumb.Item active>Thêm lịch khám</Breadcrumb.Item>
                </Breadcrumb>
                {visibleAlert?
