...
import { faBell, faCog, faEnvelopeOpen, faSearch, faSignOutAlt, faTrash, faUserCircle,faCalendarAlt } from "@fortawesome/free-solid-svg-icons";
...
export default function Booking() {
  ...
  const handleCloseShowDetail = () => {
    setChosenService([])
    dispatch.appointment.showBooking(false);
  } 
  ...
  const save = async ()=>{
    ...
    if(passed)
    {
      ...
      await Appointment.addAppointment(data).then(async result=>{
        ...
        if(result.result=="okie")
        {
          ...
          if (location.pathname.includes('my-appointment')) {
            await Appointment.getAppointments().then(data =>{
                if(data.result=="okie")
                {
          ...
          {
            navigate(Routes.MyAppointment.path,{replace:true})
          }
        }
      })
      setChosenService([])
  ...
  const { doctorId, serviceId, isBooking, userLogin, services , doctors} = useSelector(state => {
    return {
      isBooking: state.appointment.isBooking,
      userLogin:  state.user.userLogin,
      doctors:  state.doctor.doctors.data,
      services:  state.service.services.data.filter(item=>item.status==1),
  ...
  return (
    <>
    <Modal scrollable centered size="lg" show={isBooking} className="modal-secondary" onHide={handleCloseShowDetail}>
            <Modal.Header>
            <Modal.Title>
                        ...
                        <Row>
                            ...
                            <Row className="mb-3">
                                <Form.Group id="name">
                                    ...
                                    <Form.Control value={userLogin.full_name} disabled
                                    required type="text" />
                                </Form.Group>
                            </Row>
                            </Col>
                            ...
                            <Form.Group id="status">
                                ...
                                <Form.Control value={userLogin.email} disabled
                                    required type="text" />
                            </Form.Group>
                            </Col>
                        </Row>
