...
//  console.log(userLogin)
  ...
  let from = location.state?.from?.pathname || Routes.ServiceList.path
  ...
  const handleSubmit = async (event) => {
    ...
    if(passed)
    {
      ...
      {
        console.log(from)
        if(response.user.code=="PATIENT")
        {
          setMessageAlert("Bạn không được phép truy cập")
          handleVisible()
        }
        else
        {
        ...
        await dispatch.medicine.loadData()
      }
      else
      {
  ...
  const redirect = (e) => {
    e.preventDefault()
    navigate(Routes.Signup.path,{state: {fromScreen: "admin"}})
  }
  return (
    <main>
